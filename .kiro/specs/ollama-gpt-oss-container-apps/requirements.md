...
3. THE Deployment_Script SHALL Container_Appのイングレスを有効にし、外部トラフィックを許可する。イングレスのターゲットポートは、Ollama_Containerの11434ではなくAuth_Proxy_Containerが待ち受けるポート（8080）に設定する（詳細はRequirement 4.3）。
...
2. WHEN Deployment_Scriptの全工程が完了したとき, THE Deployment_Script SHALL Public_EndpointのURLと有効なAPI_Keyを用いたcurlコマンドの実行例をコンソール出力に表示する。デプロイが完了する前にはこの実行例を表示しない。この実行例は、利用者の主な実行環境であるWindows PowerShellでそのまま実行できる形式（`curl.exe` を明示し、JSON本体の二重引用符を `\"` にエスケープし、`"stream":false` を付与する）とする。
3. WHEN Deployment_ScriptがPublic_Endpointを取得したとき, THE Deployment_Script SHALL Public_EndpointのURL・API_Key・Ollamaモデル名・準備状態（STATUS）を `result-endpoint.md` ファイルに書き出し、デプロイの全工程が完了した時点でcurl実行例を含む完了状態（STATUS: Ready）に更新する。このファイルはAPI_Keyを含む秘密情報であるため、バージョン管理の対象から除外する。
4. THE リポジトリ SHALL `result-endpoint.md` を読み込み、有効なAPI_Keyを付与して `/api/generate` エンドポイントと対話するテキストチャットCLI（`chat.py`、Python標準ライブラリのみで動作し追加インストール不要）を含む。これは利用者がエンドツーエンドのAPI呼び出しを手元で確認するための補助ツールである。
